...
// Parse input into (x,y, dx,dy) tuples
let observations = exec(regex: "position=<\\s*([-0-9]+),\\s*([-0-9]+)> velocity=<\\s*([-0-9]+),\\s*([-0-9]+)>$",
                        againstMultipleLines: stringInput)
    .map { (Int($0[0])!, Int($0[1])!, Int($0[2])!, Int($0[3])!) }

//for i in 0..<20 {
//  print("Obs 0:",observations[i])
//}

// Generate a sky rendering for a particular point in time
// or return a null array if the resulting sky would be
// way too large (>300x300).  Runtime without the size
// cutoff is prohibitively large.  Actual answer was only
// 65x10, so we could actually have used the compactness
// of the sky as the sole heuristic
func pointsToSky( _ t : Int ) -> [[Bool]] {
    var points = [(Int,Int)]()
    for o in observations {
        let(x,y,dx,dy) = o
        let xnow = x + (t*dx)
        let ynow = y + (t*dy)
        points.append( (xnow,ynow) )
    }
    
    let xmin = Int( points.map {$0.0} .min(by: <)! )
    let xmax = Int(points.map {$0.0} .min(by: >)! + 1)
    let xsize = xmax - xmin
    
    let ymin = points.map {$0.1} .min(by: <)!
    let ymax = points.map {$0.1} .min(by: >)! + 1
    let ysize = ymax - ymin
    
    if (xsize > 300) || (ysize > 300) {
        //print("Skipping sky at t=\(t) because it was \(xsize)x\(ysize)")
        return []
    }
    //print(xmin,xmax,ymin,ymax)
    // 30 mins working to get regex running, tested, and write plotSky points conversion and minmax
    
    var sky : [[Bool]] = Array<[Bool]>(repeating: Array<Bool>(repeating: false, count: xsize), count: ysize)
    
    for p in points {
        sky[p.1-ymin][p.0-xmin] = true
    }
    
    return sky
}

// Render a sky to the console
func printSky( _ sky : [[Bool]] ) {
    for y in 0..<sky.count {
        let line = String( sky[y].map {$0 ? "#" : "."} )
        print( line )
    }
    // 45 mins working to get sky display running
}

// Secondary heuristic for whether a sky might reflect a readable
// pattern: how many horizontal line segments does it have?
func mightBeReadable( _ sky : [[Bool]] ) -> Int {
    // Heuristic for whether the sky passed might have text in it
    ...
    //print("Found \(horizontallines) h lines")
    return horizontallines
}

//let testsky = pointsToSky(3)
//printSky( testsky )
//mightBeReadable(testsky)

// Test timepoints starting at 0 by both size and horizontal-lines
// heuristics and print the promising ones out
for i in 0..<10070 {
    let sky = pointsToSky(i)
    if sky.count > 0 {
        let score = mightBeReadable(sky)
        if score > 2 {
            print(i)
            printSky(sky)
        }
...
// Finished first full run at about 60 minutes work, but my score criteria were
// too stringent to see the answer.  Though I could see the size approached a
// minimum around 10060.  I loosened the second heuristic and had the answer in
// another minute.
//
// Had Part II answer in another minute after adding "print(i)".

// In retrospect, I could have saved a little time using fixed-width parsing
// on the input as I had to debug the regex for 10-15 minutes due to forgetting
// that /d does not include '-'
